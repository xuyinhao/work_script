...
class myThread(threading.Thread): #继承父类threading.Thread
	def __init__(self,threadname,c,num):
		threading.Thread.__init__(self)  #
		self.name = threadname
		self.cyc = c
		self.num = num
	def run(self):  #要执行的代码写在run函数里面 线程在创建后台直接运行run函数
		opFile(self.name,self.cyc,self.num)
...
def opFile(thrname,cyc,tnum):
	...
	try:
		...
		if optype == '1':
			...
			f.seek((tnum-1)*fsize,0)
			print thrname,(tnum-1)*fsize
...
if __name__ == "__main__":
	if len(sys.argv) != 6:
		print "\033[31;1m Usage:filename optype(1:write, 2:read) filesize(k,m,g,t) blocksize(k,m,g,t) thread_num\033[0m"
		sys.exit(1)
	#if sys.argv[2] == '1' :
	#:
	#	mkdir(sys.argv[1])    
	...
	for c in range(1,cyc+1):
		...
		try:
			for i in range(1,thread+1):
				threadname = str(i)
				threads.append(myThread(threadname,c,i))
			for t in threads:
				t.start()
			for t in threads:
				t.join()
